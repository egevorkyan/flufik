...
><b>Information:</b> Flufik version latest(main version: 0.2) has lots of new features,
> in this version added features related pgp. Now 0.2 version users don't need to put in 
> config.yaml passPhrase or put private key file path. Before using it import pgp key to flufik 
> if using own pgp and in config just put name of pgp imported. If pgp is generated by flufik
> no need to import pgp, just use name of pgp in config.yaml. Pgp keys generated by flufik can be exported 
> to files, check documentation. 
